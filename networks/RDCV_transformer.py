...
class RDCV_QueryTr(nn.Module):
    def __init__(self, in_channels, embedding_dim=128, patch_size=16, num_heads=4, query_nums=100, dim_out=256, norm='linear',
        ...
        super(RDCV_QueryTr, self).__init__()
        self.norm = norm
    ...
    def forward(self, x0):
        ...
        embeddings_0 = embeddings_0.flatten(2) # (B, C, N_patches)
        ...
        # Local convolutional features for pixel-query matching
        x0 = self.conv3x3(x0)
        ...
        if self.norm == 'linear':
            ...
            return torch.softmax(y, dim=1), energy
        else:
            y = torch.sigmoid(y)
