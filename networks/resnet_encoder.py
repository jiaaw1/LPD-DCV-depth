...
class Lap_decoder(nn.Module):
    def __init__(self, dimList):
        super(Lap_decoder, self).__init__()
        norm = 'BN'
        act = 'ReLU'
    ...
    def forward(self, x, rgb):
        ...
        dense_feat = self.ASPP(dense_feat) # M5
        ...
        dec2_up = self.decoder2_1(torch.cat([dec2, lap_lv5_up, rgb_lv4], dim=1)) # M4
        dec2 = self.decoder2_2(dec2_up)
        dec2 = self.decoder2_3(dec2)

        # C-RMF
        lv4_m = 0.1 * rgb_lv4.mean(dim=1, keepdim=True)
        lap_lv4 = torch.tanh(self.decoder2_4(dec2) + lv4_m) # R4
        ...
        dec3_up = self.decoder2_1_1(torch.cat([dec3, lap_lv4_up, rgb_lv3], dim=1)) # M3
        dec3 = self.decoder2_1_2(dec3_up)

        # C-RMF
        lv3_m = 0.1 * rgb_lv3.mean(dim=1, keepdim=True)
        lap_lv3 = torch.tanh(self.decoder2_1_3(dec3) + lv3_m) # R3
        ...
        dec4_up = self.decoder2_1_1_1(torch.cat([dec4, lap_lv3_up, rgb_lv2], dim=1)) # M2

        # C-RMF
        lv2_m = 0.1 * rgb_lv2.mean(dim=1, keepdim=True)
        lap_lv2 = torch.tanh(self.decoder2_1_1_2(dec4_up) + lv2_m) # R2

        # CRC
        lap_lv2_up = self.upscale(lap_lv2, scale_factor=2, mode='bilinear')
        dec5 = self.decoder2_1_1_1_up4(dec4_up)
        dec5 = self.decoder2_1_1_1_1(torch.cat([dec5, lap_lv2_up, rgb_lv1], dim=1)) # M1
        dec5 = self.decoder2_1_1_1_2(dec5)
        # C-RMF
        lv1_m = 0.1 * rgb_lv1.mean(dim=1, keepdim=True)
        lap_lv1 = torch.tanh(dec5 + lv1_m)
...
class ResnetEncoderDecoder(nn.Module):
    def __init__(self, num_layers=50, num_features=512, model_dim=32):
        ...
        self.decoder = Lap_decoder(dimList=[64, 256, 512, 1024])
